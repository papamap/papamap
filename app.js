...
async function fetchWeather(lat, lng) {
    ...
    try {
        ...
        window.globalWeatherHtml = `<div onclick="window.open('https://weather.naver.com/', '_blank')" style="cursor:pointer; display:flex; align-items:center; gap:4px; background:rgba(241,243,245,0.8); padding:4px 10px; border-radius:12px; font-size:12px; font-weight:700; color:#495057;">${icon} ${temp}°C | ${aqiIcon} ${aqiText}</div>`;
        ...
        if ((isBadAir || isRaining) && activeCategory === '전체' && !window.weatherRecoShown) {
            ...
            if(banner) {
                banner.innerHTML = `오늘은 ${isRaining ? '비가 오네요 ☔' : '미세먼지가 나빠요 😷'}<br>실내를 살펴보는 게 어떨까요?`;
                banner.style.display = 'block';
                setTimeout(() => { banner.style.display = 'none'; setCategory('실내'); }, 3000);
            }
        }
...
async function incrementViewCount(id) {
    ...
    if (!place) return;
    const lastViewed = localStorage.getItem('last_view_' + id); const now = Date.now();
    if (lastViewed && (now - parseInt(lastViewed)) < 1000 * 60 * 5) return;
...
function updateVisibleMarkers() {
    if (!map) return;
    const bounds = map.getBounds(); const isZoomedOut = map.getZoom() < 13;
    ...
    if (activeCategory === '전체') { renderTop5(bounds); } 
    else { const top5Container = document.getElementById('top5-container'); if (top5Container) top5Container.style.display = 'none'; }
}

function renderTop5(bounds) {
    const visiblePlaces = placesData.filter(p => bounds.hasLatLng(new naver.maps.LatLng(p.latitude, p.longitude)));
    ...
    if (!container) { container = document.createElement('div'); container.id = 'top5-container'; container.className = 'top5-overlay'; document.body.appendChild(container); }
    if (top5.length === 0) { container.style.display = 'none'; return; }
    
    container.style.display = 'block';
    container.innerHTML = `
        <div class="top5-header">🔥 조회수 top 5</div>
        <div class="top5-list">
            ${top5.map((p, idx) => `
...
async function loadMainPopup() {
    ...
    if(data && data.length > 0) {
        const validPopup = data.find(n => !n.popup_end_date || new Date(n.popup_end_date) >= new Date());
        ...
        if ((!query || nameMatch || catMatch) && inScope && isCatActive) {
            ...
            listEl.innerHTML += `
                <li class="search-result-item" onclick="switchTab('map'); map.setZoom(15); map.panTo(new naver.maps.LatLng(${p.latitude}, ${p.longitude})); renderPanel(${p.id});">
                    ...
                    <div style="font-size:11px; color:#868e96; display:flex; align-items:center;">${pCat} ${distText}</div>
                </li>`;
            resultCount++;
        }
    });
...
function renderPanel(id) {
    ...
    const top5Container = document.getElementById('top5-container'); if(top5Container) top5Container.style.display = 'none';
...
async function submitEditInfo() {
    ...
    const btnSave = document.querySelector('#edit-modal .btn-save'); btnSave.innerText = "요청 접수 중..."; btnSave.disabled = true;
    ...
    // 장소를 바로 비활성화하지 않고 문의사항(inquiries) DB로 요청 내용 전달 (기존 정보 유지)
    const { error } = await supabaseClient.from('inquiries').insert([{
        content: `[장소수정요청 ID:${id}] 변경내용: ` + JSON.stringify(updatePayload),
        contact_info: '시스템 자동접수'
    }]);

    if(!error) {
        alert("수정이 접수되었습니다.\n관리자 확인 후 반영되며, 기존 정보는 정상적으로 계속 서비스됩니다.");
        document.getElementById('edit-modal').style.display = 'none';
    } else alert("업데이트 접수 실패: " + error.message);
