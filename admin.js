...
async function submitAdminLogin() {
    ...
    if (error) { errorEl.innerText = "로그인 정보가 일치하지 않습니다."; errorEl.style.display = 'block'; } else { showAdminMain(); }
}
...
async function adminLogout() { await supabaseClient.auth.signOut(); location.reload(); }
...
async function loadAdminPlaces() {
    ...
    if (!error && data) { adminPlaces = data; renderAdminTable(); updateStats(); } else alert("데이터 로드 오류: " + error.message);
}
...
function updateStats() {
    ...
    if(adminBoardData.length > 0) {
        const sortedBoard = [...adminBoardData].sort((a,b) => (b.views || 0) - (a.views || 0)); // 게시글도 views 지원시
        document.getElementById('stat-top-board').innerText = `${sortedBoard[0].title}`;
    }
}
...
function renderAdminTable() {
    ...
    document.getElementById('cb-all').checked = false; updateBulkBar();

    if (filtered.length === 0) { tbody.innerHTML = `<tr><td colspan="9" style="text-align:center; padding: 40px; color:#adb5bd;">데이터가 없습니다.</td></tr>`; return; }
...
document.getElementById('admin-tbody').addEventListener('keydown', function(e) {
    ...
    const cellIndex = Array.from(tr.children).indexOf(td); const rowIndex = Array.from(tbody.children).indexOf(tr); let nextInput = null;
...
async function bulkAction(actionType) {
    const checked = document.querySelectorAll('.cb-item:checked'); if (checked.length === 0) return;
    const ids = Array.from(checked).map(cb => parseInt(cb.value));
    if (actionType === 'category') {
        const newCat = document.getElementById('bulk-category').value; if (!confirm(`선택한 ${ids.length}개 장소를 '${newCat}'(으)로 변경하시겠습니까?`)) return;
        ...
        if (!confirm(`선택한 ${ids.length}개 장소를 노출(승인)하시겠습니까?`)) return;
...
async function quickSave(id, btnElement) {
    ...
    if(!error) {
        ...
        btnElement.innerText = "✔ 완료"; btnElement.style.background = "#20c997"; setTimeout(() => { btnElement.innerText = originalText; btnElement.style.background = "#5c7cfa"; btnElement.disabled = false; }, 1500);
    } else { alert("수정 에러: " + error.message); btnElement.disabled = false; }
}

async function approvePlace(id) { const { error } = await supabaseClient.from('places').update({ is_approved: true }).eq('id', id); if(!error) { adminPlaces.find(x => x.id === id).is_approved = true; renderAdminTable(); } }
async function deletePlace(id) { if(!confirm("이 장소를 삭제하시겠습니까?")) return; const { error } = await supabaseClient.from('places').delete().eq('id', id); if(!error) { adminPlaces = adminPlaces.filter(p => p.id !== id); renderAdminTable(); } }

function loadAllComments() {
    const tbody = document.getElementById('comments-tbody'); let allComments = [];
    adminPlaces.forEach(p => { if(p.comments_list) { try { const arr = JSON.parse(p.comments_list); arr.forEach(c => { allComments.push({ placeId: p.id, placeName: p.name, ...c }); }); } catch(e){} } });
    ...
    tbody.innerHTML = allComments.map(c => `<tr><td style="font-weight:700; color:#5c7cfa;">${escapeHtml(c.placeName)}</td><td style="font-weight:600;">${escapeHtml(c.author)}</td><td>${escapeHtml(c.text)}</td><td style="font-size:12px; color:#868e96;">${timeAgo(c.date || c.id)}</td><td><button class="btn btn-del" onclick="deleteCommentByAdmin(${c.placeId}, ${c.id})">삭제</button></td></tr>`).join('');
}
async function deleteCommentByAdmin(placeId, commentId) {
    if(!confirm("댓글을 삭제하시겠습니까?")) return;
    const place = adminPlaces.find(p => p.id === placeId); let comments = place.comments_list ? JSON.parse(place.comments_list) : [];
    comments = comments.filter(c => c.id !== commentId); const updatedJson = JSON.stringify(comments);
    const { error } = await supabaseClient.from('places').update({ comments_list: updatedJson }).eq('id', placeId);
    if(!error) { place.comments_list = updatedJson; loadAllComments(); } else alert("삭제 에러");
}

async function loadInquiries() {
    const tbody = document.getElementById('inquiries-tbody'); const { data, error } = await supabaseClient.from('inquiries').select('*').order('created_at', { ascending: false });
    if (!error && data) { adminInquiries = data; renderInquiriesTable(); } else tbody.innerHTML = `<tr><td colspan="4" style="text-align:center; padding: 40px;">데이터가 없습니다.</td></tr>`;
}
function renderInquiriesTable() {
    const tbody = document.getElementById('inquiries-tbody');
    if (adminInquiries.length === 0) { tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;">수정요청/문의가 없습니다.</td></tr>`; return; }
    tbody.innerHTML = adminInquiries.map(i => `<tr><td style="color:#adb5bd; font-weight:700;">#${i.id}</td><td style="color:#868e96; font-size:12px;">${new Date(i.created_at).toLocaleString('ko-KR')}</td><td><div style="white-space: pre-wrap;">${escapeHtml(i.content)}</div></td><td style="color:#5c7cfa; font-weight:600;">${escapeHtml(i.contact_info || '-')}</td></tr>`).join('');
}
...
async function loadAdminBoard() {
    const tbody = document.getElementById('board-tbody'); tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding:40px;">불러오는 중...</td></tr>`;
    const { data, error } = await supabaseClient.from('notices').select('*').order('created_at', { ascending: false });
    if (!error && data) { adminBoardData = data; updateStats(); renderAdminBoard(); } else tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">게시글이 없습니다.</td></tr>`;
}
function renderAdminBoard() {
    const tbody = document.getElementById('board-tbody');
    if (adminBoardData.length === 0) { tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">게시글이 없습니다.</td></tr>`; return; }
    tbody.innerHTML = adminBoardData.map(n => {
        const noticeBtn = n.is_notice ? `<button class="btn btn-save" style="background:#FF6B6B;" onclick="toggleNotice(${n.id}, false)">📌 공지 해제</button>` : `<button class="btn btn-save" style="background:#adb5bd;" onclick="toggleNotice(${n.id}, true)">공지 설정</button>`;
        const popupBtn = n.is_popup ? `<button class="btn btn-save" style="background:#f59f00;" onclick="togglePopup(${n.id}, false)">🎈 팝업 해제</button>` : `<button class="btn btn-save" style="background:#adb5bd;" onclick="togglePopup(${n.id}, true)">팝업 설정</button>`;
        return `
        <tr>
            <td style="color:#adb5bd; font-weight:700;">#${n.id}</td>
            <td style="text-align:center; display:flex; gap:4px; flex-direction:column;">${popupBtn} ${noticeBtn}</td>
            <td style="font-weight:600;">${escapeHtml(n.author || '익명')}</td>
            <td style="font-weight:700;">${escapeHtml(n.title)}</td>
            <td style="font-size:12px; color:#868e96;"><input type="date" value="${n.popup_end_date || ''}" onchange="updatePopupDate(${n.id}, this.value)" style="padding:4px; border:1px solid #dee2e6; border-radius:6px;"></td>
            <td><button class="btn btn-del" onclick="deleteAdminBoard(${n.id})">삭제</button></td>
        </tr>`}).join('');
}

async function toggleNotice(id, state) { const { error } = await supabaseClient.from('notices').update({ is_notice: state }).eq('id', id); if (!error) { adminBoardData.find(x => x.id === id).is_notice = state; renderAdminBoard(); } }
async function togglePopup(id, state) { const { error } = await supabaseClient.from('notices').update({ is_popup: state }).eq('id', id); if (!error) { adminBoardData.find(x => x.id === id).is_popup = state; renderAdminBoard(); } }
async function updatePopupDate(id, dateVal) { const { error } = await supabaseClient.from('notices').update({ popup_end_date: dateVal || null }).eq('id', id); if(!error) alert("팝업 종료일이 업데이트 되었습니다."); }
async function deleteAdminBoard(id) { if(!confirm("이 게시글을 삭제하시겠습니까?")) return; const { error } = await supabaseClient.from('notices').delete().eq('id', id); if (!error) { adminBoardData = adminBoardData.filter(x => x.id !== id); renderAdminBoard(); } }